...
Keyword arguments:
- `children` (a list of or a singular dash component, string or number; optional): Drawer contents.
...
- `hasBackdrop` (Bool; optional): Whether a container-spanning backdrop element should be rendered behind the contents. When false, users 
...
- `position` (String; optional): Position of a drawer. All angled positions will be casted into pure positions (TOP, BOTTOM, LEFT or RIGHT).
...
closes.
- `size` (Real | a value equal to: 'small', 'standard', 'large'; optional): CSS size of the drawer. This sets width if horizontal position (default) and height otherwise.
...
function drawer(; kwargs...)
        available_props = Symbol[:children, :id, :autoFocus, :backdropClassName, :canEscapeKeyClose, :canOutsideClickClose, :className, :enforceFocus, :hasBackdrop, :icon, :isCloseButtonShown, :isOpen, :lazy, :portalClassName, :portalContainer, :position, :shouldReturnFocusOnClose, :size, :style, :title, :transitionDuration, :transitionName, :usePortal]
        wild_props = Symbol[]
        return Component("drawer", "Drawer", "dash_blueprint_components", available_props, wild_props; kwargs...)
end
